...
public class AdaptProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        processingEnv.getMessager().printMessage(Kind.WARNING,"begin ddd");
        for (TypeElement annotation : annotations) {
            if (!"annotation.codegener.Adapt".equals(annotation.getQualifiedName().toString())) {
                continue;
            }
    ...
    private void createAdapter(TypeElement enclosingClass, ExecutableElement annotatedMethod,
        ...
        try {
            processingEnv.getMessager().printMessage(Kind.WARNING,"begin cg");
            ...
            try {
                String className1 = packageName + "." +adapterName;
                processingEnv.getMessager().printMessage(Kind.WARNING,"begin :className      "+className1);

                Class<?> aClass = getClass().getClassLoader().loadClass(className1);
                String s = aClass.newInstance().toString();
                processingEnv.getMessager().printMessage(Kind.WARNING,"begin :tostring"+s);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } catch (IOException e) {
